...
from custom_components.sense_realtime.const import DOMAIN
...
def mock_sense():
    ...
    with patch(
        "custom_components.sense_realtime.config_flow.ASyncSenseable"
    ) as mock_sense:
...
async def test_form(hass: HomeAssistant, mock_sense) -> None:
    ...
    with patch(
        "custom_components.sense_realtime.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
